...
Waddler provides a convinient [`sql.default`](/docs/mssql/sql-default) for SQL `DEFAULT` values:
```ts
...
Complete example with [`sql.identifier`](/docs/mssql/sql-identifier) and [`sql.default`](/docs/mssql/sql-default):
<Section>
```ts
