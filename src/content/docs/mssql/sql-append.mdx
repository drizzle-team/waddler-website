...
```
```sql
select * from items limit @p1; -- params: [50]
select * from items where [name] = @p1 limit @p2; -- params: ["Item 1", 50]
select * from items where [name] = @p1 limit @p2 offset @p3; -- params: ["Item 1", 50, 50]
```
</Section>