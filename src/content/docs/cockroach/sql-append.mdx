...
```
```sql
select * from items limit $1::int4; -- params: [50]
select * from items where "name" = $1 limit $2::int4; -- params: ["Item 1", 50]
select * from items where "name" = $1 limit $2::int4 offset $3::int4; -- params: ["Item 1", 50, 50]
```
</Section>