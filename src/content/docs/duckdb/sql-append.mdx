...
```
```sql
select * from items limit $1; -- params: [50]
select * from items where "name" = $1 limit $2; -- params: ["Item 1", 50]
select * from items where "name" = $1 limit $2 offset $3; -- params: ["Item 1", 50, 50]
```
</Section>